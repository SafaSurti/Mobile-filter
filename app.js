...
function searchMobile() {
    ...
    if (brand && model && mobiles[brand][model]) {
        ...
        resultDiv.innerHTML = `
            ...
            <p><strong>RAM:</strong> ${mobile.memory?.ram || "N/A"}GB</p>
            <p><strong>Storage:</strong> ${mobile.memory?.storage || "N/A"}GB</p>
            <p><strong>Battery:</strong> ${mobile.battery || "N/A"}mAh</p>
            <p><strong>Operating System:</strong> ${mobile.operatingSystem || "N/A"}</p>
            <p><strong>Rear Camera:</strong> ${
                mobile.camera?.rear 
                    ? Object.entries(mobile.camera.rear).map(([key, val]) => `${key}: ${val}`).join(", ") 
                    : "N/A"
            }</p>
            <p><strong>Front Camera:</strong> ${mobile.camera?.front || "N/A"}MP</p>
            <p><strong>Special Features:</strong> ${mobile.specialFeatures?.join(", ") || "N/A"}</p>
        `;
    } else {
        resultDiv.innerHTML = `<p>Please select a valid brand and model.</p>`;
    }
}
